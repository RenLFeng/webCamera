...
<template>
  <div id="routerManage" class="router-manage">
    <div class="manage-left" v-if="!hideLeft">
    ...
    <router-view @set-manage-menu="getMaintainanageMenu"></router-view>
  </div>
</template>
...
<script>
  export default {
    ...
    data() {
      return {
        hideLeft:false,
        menuName: "masterControl",
        originMenuList: [
          { name: "masterControl", text: "系统看板" },
          { name: "deviceLog", text: "监控列表" },
          { name: "deviceAbnormal", text: "故障处理" },
          { name: "todayControl", text: "今日看板" },
        ]
      };
    },
    beforeCreate() {
      // 传值给父级main.vue（第二个导航）
      this.$emit("set-active-nav", "maintain");
    },
    created() { },
    computed:{
      menuList(){
    ...
    methods: {
      // 接收从子组件传来的当前选中菜单
      getMaintainanageMenu(name,hideLeft) {
        this.menuName = name;
        this.hideLeft=hideLeft?hideLeft:false;
      },
      // 菜单栏跳转
      pageToMenu(name) {
        this.$router.push("/maintain/" + name);
      }
    }
  };
</script>
