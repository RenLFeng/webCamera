    ...
    <div class="box">
      ...
      <div class="box-bd">
        <div v-if="Object.keys(tableData.rows).length!=0">
          <table class="mytable">
            ...
            <tbody>
              <tr v-for="(item,index) in tableData.rows" :key="index">
                ...
                <td>{{msgtype[item.type-1]}} / {{item.title}}</td>
                <td>
          ...
          <Modal v-model="modalDetail" title="通知消息" class="msg_modal" width="640">
            <div class="title">
              <h3>
                {{detailItem.title}}
              </h3>
  ...
  export default {
    ...
    data() {
      return {
        ...
        modalDetail: false,
        detailItem: {},
        msgtype: ['产品类型审核消息 ', '产品升级消息提醒', '推送审核消息提醒'],
      }
    },
    beforeCreate() {
      // 传值给父级message.vue（选中通知消息）
      this.$emit("set-message-menu", "notice-message");
    ...
    methods: {
      ...
      detail(item) {
        ...
        if (item.isread != 1) {
          this.axios.post(this.api.editm, {
            data: {
      ...
      mockTableData() {
        ...
        this.axios.get(this.api.findmsg, {
          data
        }).then(res => {
          console.log(res, '查询通知消息');
          if (res.result == '1') {
